...
def knapsack_neglect(B,  capacity_knapsack=11,weight_item=np.array([2,4,6,7]), profit_item=np.array([6,10,12,13])):
    """ knapsack_neglect """
    if weight_item.sum() <= capacity_knapsack:
        return np.sum((profit_item * B), axis=1)
    else:
        return -10000000
def knapsack_penalty(B, capacity_knapsack=11,weight_item=np.array([2,4,6,7]), profit_item=np.array([6,10,12,13])):
    """ knapsack_penalty"""
